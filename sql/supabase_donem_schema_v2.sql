-- ================================================================================================
-- DÖNEM YÖNETİMİ VERİTABANI ŞEMASI - DÜZELTİLMİŞ VERSİYON
-- ================================================================================================
-- Bu script, mevcut Supabase veritabanınıza dönem yönetimi özelliği ekler.
-- 
-- YAPILAN İŞLEMLER:
-- 1. 'donemler' tablosunu oluşturur
-- 2. Mevcut tablolardaki 'donem' sütunlarını DATE'den VARCHAR'a çevirir
-- 3. Eksik dönemleri otomatik oluşturur
-- 4. Foreign key kısıtlamalarını ekler
-- 5. Trigger'ları ve RLS politikalarını ayarlar
-- 
-- ÖNEMLI: Bu script tip uyumsuzluklarını otomatik olarak düzeltir
-- ================================================================================================
...
-- ADIM 1: TÜM TABLOLARIN DONEM SÜTUNLARINI VARCHAR'A ÇEVİR
-- Bordro tablosu
DO $$ 
DECLARE
    col_type TEXT;
BEGIN 
    ...
    ELSIF col_type != 'character varying' THEN
        RAISE EXCEPTION 'bordro.donem sütunu beklenmeyen tipte: %', col_type;
    ELSE
...
BEGIN 
    ...
    ELSIF col_type != 'character varying' THEN
        RAISE EXCEPTION 'izinler.donem sütunu beklenmeyen tipte: %', col_type;
    ELSE
...
BEGIN 
    ...
    ELSIF col_type != 'character varying' THEN
        RAISE EXCEPTION 'mesai.donem sütunu beklenmeyen tipte: %', col_type;
    ELSE
        RAISE NOTICE 'mesai.donem sütunu zaten VARCHAR tipinde';
    END IF;
END $$;

-- Ödemeler tablosu (eğer varsa)
DO $$ 
DECLARE
    col_type TEXT;
...
BEGIN 
    ...
    IF table_exists THEN
        ...
        ELSIF col_type != 'character varying' THEN
            RAISE EXCEPTION 'odemeler.donem sütunu beklenmeyen tipte: %', col_type;
        ELSE
...
-- ADIM 2: EKSİK DÖNEMLERİ OLUŞTUR (ARTIK TÜM SÜTUNLAR VARCHAR)
DO $$
DECLARE
...
-- ADIM 3: FOREIGN KEY KISITLAMALARINI EKLE
DO $$
BEGIN
...
-- ADIM 4: RLS POLİTİKALARI
ALTER TABLE donemler ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Donemler görüntüleme" ON donemler;
CREATE POLICY "Donemler görüntüleme" ON donemler FOR SELECT USING (true);

DROP POLICY IF EXISTS "Donemler ekleme" ON donemler;
CREATE POLICY "Donemler ekleme" ON donemler FOR INSERT WITH CHECK (true);

DROP POLICY IF EXISTS "Donemler güncelleme" ON donemler;
CREATE POLICY "Donemler güncelleme" ON donemler FOR UPDATE USING (true) WITH CHECK (true);

DROP POLICY IF EXISTS "Donemler silme" ON donemler;
CREATE POLICY "Donemler silme" ON donemler FOR DELETE USING (true);

-- ADIM 5: MEVCUT KAYITLARI AKTİF DÖNEME ATA
DO $$
DECLARE
...
BEGIN
    ...
    ELSE
        RAISE WARNING 'Aktif dönem bulunamadı';
    END IF;
END $$;
