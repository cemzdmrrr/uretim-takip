-- İplik Sipariş Tablosu Schema Düzeltmesi
-- Bu script mevcut tablodaki sorunları düzeltir

-- 1. iplik_adi sütununun NOT NULL kısıtlamasını kaldır
DO $$
BEGIN
    -- iplik_adi sütununu opsiyonel hale getir
    BEGIN
        ALTER TABLE iplik_siparisleri ALTER COLUMN iplik_adi DROP NOT NULL;
        RAISE NOTICE 'iplik_adi sütunu NULL değerlere izin verecek şekilde güncellendi.';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE NOTICE 'iplik_adi sütunu zaten NULL değerlere izin veriyor veya sütun mevcut değil: %', SQLERRM;
    END;
    
    -- Eğer iplik_adi sütunu yoksa ekle (opsiyonel olarak)
    BEGIN
        ALTER TABLE iplik_siparisleri ADD COLUMN IF NOT EXISTS iplik_adi VARCHAR;
        RAISE NOTICE 'iplik_adi sütunu kontrol edildi/eklendi.';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE NOTICE 'iplik_adi sütunu eklenirken sorun: %', SQLERRM;
    END;
    
    -- Mevcut boş kayıtları temizle (eğer varsa)
    DELETE FROM iplik_siparisleri WHERE siparis_no = 'SIP-2025-001';
    RAISE NOTICE 'Mevcut test verileri temizlendi.';
    
END $$;

-- 2. Tabloların varlığını kontrol et ve eksikse oluştur
...
-- 3. Trigger fonksiyonlarını yeniden oluştur
...
-- 7. Örnek veri oluştur (güvenli şekilde)
DO $$
DECLARE
    siparis_id UUID;
    kalem_id1 UUID;
    ...
    mevcut_siparis_count INTEGER;
BEGIN
    -- Tedarikçi var mı kontrol et
    SELECT COUNT(*) INTO tedarikci_count FROM tedarikciler WHERE durum = 'aktif';
    
    -- Aynı sipariş no var mı kontrol et
    SELECT COUNT(*) INTO mevcut_siparis_count FROM iplik_siparisleri WHERE siparis_no = 'SIP-2025-001';
    
    IF tedarikci_count > 0 AND mevcut_siparis_count = 0 THEN
        -- Önce ana sipariş oluştur
        INSERT INTO iplik_siparisleri (
            ...
            'onaylandi',
            'Örnek karışık renk iplik siparişi - Takip sistemi demo'
        ...
        (kalem_id1, 'TES-001-' || EXTRACT(EPOCH FROM NOW())::INTEGER, CURRENT_DATE - INTERVAL '5 days', 50.00, 'LOT-2025-001', 'onaylandi'),
        (kalem_id2, 'TES-002-' || EXTRACT(EPOCH FROM NOW())::INTEGER, CURRENT_DATE - INTERVAL '3 days', 80.00, 'LOT-2025-002', 'onaylandi');
        
        RAISE NOTICE 'Demo sipariş ve teslimat verileri başarıyla oluşturuldu.';
    ELSE
        IF tedarikci_count = 0 THEN
            RAISE NOTICE 'Aktif tedarikçi bulunamadı. Önce tedarikçi ekleyin.';
        END IF;
        IF mevcut_siparis_count > 0 THEN
            RAISE NOTICE 'SIP-2025-001 numaralı sipariş zaten mevcut.';
        END IF;
    END IF;
    
EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE 'Demo veri oluşturulurken hata: %', SQLERRM;
END $$;

-- İplik sipariş takip sistemi başarıyla kuruldu!
SELECT 'İplik sipariş takip sistemi başarıyla kuruldu!' as kurulum_durumu;
