-- İplik ve aksesuar stok tabloları oluşturma (GÜÇLÜ VERSİYON)
-- Bu dosyayı Supabase SQL editörde çalıştırın
-- UYARI: Mevcut aksesuarlar tablosunu silip yeniden oluşturur!

-- Mevcut tabloları temizle (isteğe bağlı)
DROP TABLE IF EXISTS public.aksesuar_hareketleri CASCADE;
DROP TABLE IF EXISTS public.iplik_hareketleri CASCADE;
DROP TABLE IF EXISTS public.aksesuarlar CASCADE;
DROP TABLE IF EXISTS public.iplik_stoklari CASCADE;
...
DO $$
BEGIN
    RAISE NOTICE 'STOK TABLOLARİ BAŞARIYLA OLUŞTURULDU!';
    RAISE NOTICE 'İplik stokları: % adet', (SELECT COUNT(*) FROM public.iplik_stoklari);
    RAISE NOTICE 'Aksesuarlar: % adet', (SELECT COUNT(*) FROM public.aksesuarlar);
END $$;
