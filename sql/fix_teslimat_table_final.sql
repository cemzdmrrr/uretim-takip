-- İplik teslimat kayıtları tablosunu düzelt
-- siparis_id yerine kalem_id kullanacak şekilde güncelle
...
-- 2. siparis_id kolonunu sil ve kalem_id ekle
ALTER TABLE iplik_teslimat_kayitlari 
DROP COLUMN IF EXISTS siparis_id;

-- kalem_id kolonu yoksa ekle
DO $$
BEGIN
    IF NOT EXISTS (
        ...
        RAISE NOTICE 'kalem_id kolonu eklendi';
    ELSE
        RAISE NOTICE 'kalem_id kolonu zaten mevcut';
    END IF;
END $$;

-- 3. Doğru foreign key constraint'i ekle
ALTER TABLE iplik_teslimat_kayitlari
ADD CONSTRAINT fk_teslimat_kalem 
FOREIGN KEY (kalem_id) REFERENCES iplik_siparis_kalemleri(id) ON DELETE CASCADE;

-- 4. Trigger fonksiyonlarını yeniden oluştur
...
-- 7. Test sorgusu
SELECT 
    'Düzeltme tamamlandı!' as durum,
    (SELECT COUNT(*) FROM iplik_teslimat_kayitlari) as teslimat_kayit_sayisi,
    (SELECT COUNT(*) FROM iplik_siparis_kalemleri) as kalem_sayisi;
