-- ============================================
-- ALTERNATIF VERSİYON: ROL KONTROLÜ İLE
-- Önce bu komutu çalıştırarak personel tablosu yapısını kontrol edin:
-- SELECT column_name, data_type FROM information_schema.columns WHERE table_name = 'personel';
-- ============================================
...
CREATE TABLE personel_donem (
    ...
    kalan_yillik_izin INTEGER DEFAULT 0, -- Yıllık izin kalanı (her dönem taşınır)
...
-- 7. Yazma politikaları - EĞER 'pozisyon' SÜTUNU VARSA BU VERSİYONU KULLANIN
-- CREATE POLICY "Sadece admin donemler tablosunu değiştirebilir" ON donemler
--     FOR ALL USING (
--         EXISTS (
--             SELECT 1 FROM personel 
--             WHERE user_id = auth.uid() 
--             AND pozisyon IN ('admin', 'yonetici', 'ik')
--         )
--     );

-- 7b. Basit yazma politikaları (tüm authenticated kullanıcılar)
...
SELECT 'Oluşturulan tablolar:' as bilgi;
SELECT table_name FROM information_schema.tables 
WHERE table_schema = 'public' 
AND table_name IN ('donemler', 'personel_donem');

SELECT 'Eklenen dönemler:' as bilgi;
