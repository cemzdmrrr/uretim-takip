...
CREATE TABLE IF NOT EXISTS public.yukleme_kayitlari (
    ...
    kaynak TEXT DEFAULT 'manual', -- manual, ceki_listesi, vs
...
-- Eski VIEW'ları kaldır
DROP VIEW IF EXISTS public.musteri_siparis_ozet CASCADE;

-- Müşteri sipariş özeti (view - opsiyonel, sonra elle oluşturabilirsiniz)
...
--     COALESCE(SUM(t.adet), 0) as toplam_adet,
--     COALESCE(SUM(t.yuklenen_adet), 0) as toplam_yuklenen_adet,
