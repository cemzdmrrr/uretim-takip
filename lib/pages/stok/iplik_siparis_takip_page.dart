...
class _IplikSiparisTakipPageState extends State<IplikSiparisTakipPage> {
  ...
  Widget build(BuildContext context) {
    final filtreliSiparisler = siparisler.where((siparis) {
      ...
      return siparis['siparis_no']?.toString().toLowerCase().contains(arama) == true ||
             siparis['iplik_adi']?.toString().toLowerCase().contains(arama) == true ||
             siparis['tedarikci_adi']?.toString().toLowerCase().contains(arama) == true;
    }).toList();
      ...
      body: Column(
        children: [
          ...
          Container(
            ...
            child: Row(
              children: [
                Expanded(child: _buildIstatistikKart('Toplam', siparisler.length.toString(), Colors.blue)),
                const SizedBox(width: 8),
                Expanded(child: _buildIstatistikKart('Beklemede', 
                  siparisler.where((s) => s['takip_durumu'] == 'beklemede').length.toString(), Colors.orange)),
                const SizedBox(width: 8),
                Expanded(child: _buildIstatistikKart('Tamamlanan', 
                  siparisler.where((s) => s['takip_durumu'] == 'tamamlandi').length.toString(), Colors.green)),
                const SizedBox(width: 8),
                Expanded(child: _buildIstatistikKart('Geciken', 
                  siparisler.where((s) => s['takip_durumu'] == 'gecikti').length.toString(), Colors.red)),
              ],
            ),
          ),
  ...
  Widget _buildSiparisKart(Map<String, dynamic> siparis) {
    ...
    final teslimYuzdesi = (siparis['teslim_yuzdesi'] as num?)?.toDouble() ?? 0.0;
    
    // Miktar hesaplamaları
    final siparisMiktari = (siparis['miktar'] as num?)?.toDouble() ?? 0.0;
    final teslimMiktari = (siparis['teslim_miktari'] as num?)?.toDouble() ?? 0.0;
        ...
        children: [
          ListTile(
            ...
            subtitle: Column(
              ...
              children: [
                Text('Tedarikçi: ${siparis['tedarikci_adi'] ?? 'Belirtilmemiş'}'),
  ...
  Map<String, dynamic> _getDurumBilgi(String durum) {
    switch (durum) {
      case 'beklemede':
        return {'renk': Colors.orange, 'ikon': Icons.hourglass_empty, 'metin': 'Beklemede'};
      case 'tamamlandi':
        return {'renk': Colors.green, 'ikon': Icons.check_circle, 'metin': 'Tamamlandı'};
      case 'gecikti':
              ...
              children: [
                ...
                Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Text('Toplam Miktar: ${siparisMiktari.toStringAsFixed(1)} ${siparis['birim'] ?? 'kg'}'),
                      Text('Teslim Edilen: ${mevcutTeslim.toStringAsFixed(1)} ${siparis['birim'] ?? 'kg'}'),
                      Text(
                ...
                TextField(
                  ...
                  decoration: InputDecoration(
                    ...
                    helperText: 'Maksimum: ${kalanMiktar.toStringAsFixed(1)} ${siparis['birim'] ?? 'kg'}',
                ...
                DropdownButtonFormField<String>(
                  ...
                  items: const [
                    ...
                    DropdownMenuItem(value: 'beklemede', child: Text('Kalite Kontrolü Bekliyor')),
                    DropdownMenuItem(value: 'sartli_kabul', child: Text('Şartlı Kabul')),
                    DropdownMenuItem(value: 'reddedildi', child: Text('Reddedildi')),
                  ],
                  onChanged: (value) => setState(() => kaliteDurumu = value ?? 'onaylandi'),
                ),
                const SizedBox(height: 16),
                InkWell(
                  onTap: () async {
                  ...
                  child: InputDecorator(
                    ...
                    child: Text(DateFormat('dd.MM.yyyy').format(teslimatTarihi)),
                  ),
                ),
              ],
            ),
              ...
              onPressed: () async {
                try {
                  ...
                  await supabase.from(DbTables.iplikSiparisleri).update({
                    ...
                    'lot_no': lotNoController.text.trim().isNotEmpty ? lotNoController.text.trim() : null,
                  ...
                  if (siparis['birim_fiyat'] != null) {
                    stokData['toplam_deger'] = miktar * (siparis['birim_fiyat'] as num).toDouble();
                  }
                  ...
                  await supabase.from(DbTables.iplikHareketleri).insert({
                    ...
                    'hareket_tipi': 'giris',
                    'miktar': miktar,
                    'aciklama': 'Sipariş teslimatından otomatik stok girişi - ${siparis['siparis_no']}',
                    'firma_id': TenantManager.instance.requireFirmaId,
                  });
                  ...
                  if (context.mounted) {
                    ...
                    if (teslimEdildi) {
                      ...
                      mesaj += ' Kalan miktar: ${kalanYeni.toStringAsFixed(1)} ${siparis['birim'] ?? 'kg'} (%${(100 - teslimYuzdesi).toStringAsFixed(1)})';
                    }
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                        content: Text(mesaj),
                        backgroundColor: teslimEdildi ? Colors.green : Colors.blue,
  ...
  Future<void> _siparisiBitir(Map<String, dynamic> siparis) async {
    try {
      ...
      final mevcutTeslim = (siparis['teslim_miktari'] as num?)?.toDouble() ?? 0.0;
      final kalanMiktar = siparisMiktari - mevcutTeslim;
      final teslimYuzdesi = siparisMiktari > 0 ? (mevcutTeslim / siparisMiktari) * 100 : 0.0;
      
      String mesaj = 'Siparişi tamamlandı olarak işaretlemek istediğinizden emin misiniz?\n\n';
      mesaj += 'Sipariş: ${siparis['siparis_no']}\n';
      mesaj += 'İplik: ${siparis['iplik_adi']}\n';
      mesaj += 'Toplam Miktar: ${siparisMiktari.toStringAsFixed(1)} ${siparis['birim'] ?? 'kg'}\n';
      mesaj += 'Teslim Edilen: ${mevcutTeslim.toStringAsFixed(1)} ${siparis['birim'] ?? 'kg'}\n';
      
      if (kalanMiktar > 0) {
        mesaj += '\n⚠️ DİKKAT: Sipariş tam teslim edilmemiş!\n';
        mesaj += 'Kalan Miktar: ${kalanMiktar.toStringAsFixed(1)} ${siparis['birim'] ?? 'kg'} (%${(100 - teslimYuzdesi).toStringAsFixed(1)})\n';
      ...
      if (onay == true) {
        ...
        if (mounted) {
          ...
          String basariMesaji = '✅ Sipariş ${siparis['siparis_no']} tamamlandı olarak işaretlendi!';
          if (kalanMiktar > 0) {
  ...
  void _siparisDetayGoster(Map<String, dynamic> siparis) {
    ...
    final teslimMiktari = (siparis['teslim_miktari'] as num?)?.toDouble() ?? 0.0;
    final kalanMiktar = siparisMiktari - teslimMiktari;
    final teslimYuzdesi = siparisMiktari > 0 ? (teslimMiktari / siparisMiktari) * 100 : 0.0;
    final birim = siparis['birim'] ?? 'kg';
      ...
      builder: (context) => AlertDialog(
        ...
        content: SingleChildScrollView(
          child: Column(
            ...
            mainAxisSize: MainAxisSize.min,
            children: [
              _buildDetayRow('İplik Adı', siparis['iplik_adi']?.toString() ?? '-'),
              _buildDetayRow('Renk', siparis['renk']?.toString() ?? '-'),
              
              // Miktar bilgileri - özel düzen
              const Divider(),
              const Text('📦 Miktar Bilgileri', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
              const SizedBox(height: 8),
              _buildDetayRow('Sipariş Miktarı', '${siparisMiktari.toStringAsFixed(1)} $birim'),
              _buildDetayRow('Teslim Edilen', '${teslimMiktari.toStringAsFixed(1)} $birim'),
              _buildDetayRow('Kalan Miktar', 
                '${kalanMiktar.toStringAsFixed(1)} $birim${kalanMiktar > 0 ? ' (${(100 - teslimYuzdesi).toStringAsFixed(1)}%)' : ' ✅'}',
                color: kalanMiktar > 0 ? Colors.orange : Colors.green),
              if (teslimMiktari > 0)
                _buildDetayRow('Teslim Oranı', '%${teslimYuzdesi.toStringAsFixed(1)}',
                  color: teslimYuzdesi >= 100 ? Colors.green : Colors.blue),
              
              const Divider(),
              const Text('🏪 Tedarikçi Bilgileri', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
              const SizedBox(height: 8),
              _buildDetayRow('Tedarikçi', siparis['tedarikci_adi']?.toString() ?? '-'),
              _buildDetayRow('Telefon', siparis['tedarikci_telefon']?.toString() ?? '-'),
              
              const Divider(),
              const Text('📅 Tarih Bilgileri', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
              const SizedBox(height: 8),
              _buildDetayRow('Termin Tarihi', _formatTarih(siparis['termin_tarihi'])),
              _buildDetayRow('Teslim Tarihi', _formatTarih(siparis['teslim_tarihi'])),
              
              const Divider(),
              const Text('ℹ️ Diğer Bilgiler', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
              const SizedBox(height: 8),
              _buildDetayRow('Lot No', siparis['lot_no']?.toString() ?? '-'),
              _buildDetayRow('Kalite Durumu', siparis['kalite_durumu']?.toString() ?? '-'),
              _buildDetayRow('Durum', _getDurumBilgi(siparis['takip_durumu'])['metin']),
            ],
          ),
        ),
        actions: [
