                  ...
                  child: FutureBuilder<List<dynamic>>(
                    ...
                    builder: (context, snapshot) {
                      ...
                      if (snapshot.hasData && snapshot.data != null) {
                        ...
                        debugPrint(
                            'FutureBuilder: snapshot.data NULL veya boş!');
                      }

                      // Finansal hesaplamalar
                      final netMaas = personel != null
                          ? double.tryParse(personel.netMaas) ?? 0.0
                          : 0.0;
                      final yemekUcreti = personel != null
                          ? double.tryParse(personel.yemekUcreti) ?? 0.0
                          : 0.0;
                      final toplamYemekUcreti = yemekUcreti +
                          mesaiYemekUcreti; // Personel yemek ücreti + mesai yemek ücretleri
                      final prim = (ozetBakiyeler['prim'] ?? 0).toDouble();
                      final ikramiye =
                          (ozetBakiyeler['ikramiye'] ?? 0).toDouble();
                      ...
                      return Column(
                        ...
                        children: [
                          ...
                          _buildFinancialItem('Net Maaş', netMaas,
                              isNegative: false),
                          if (mesaiUcreti > 0)
                            _buildFinancialItem('Mesailer', mesaiUcreti,
                                isNegative: false),
                          if (toplamYemekUcreti > 0)
                            _buildFinancialItem(
                                'Yemek Ücreti', toplamYemekUcreti,
                                isNegative: false),
                          if (yolUcreti > 0)
                            _buildFinancialItem('Yol Ücreti', yolUcreti,
                                isNegative: false),
                          if (ikramiye > 0)
                            _buildFinancialItem('İkramiye', ikramiye,
                                isNegative: false),
                          ...
                          const SizedBox(height: 8),
                          _buildFinancialItem('Avanslar', avans,
                              isNegative: true),
                          if (kesintiTutari > 0)
                            _buildFinancialItem('Ücretsiz İzin', kesintiTutari,
                                isNegative: true),
                          const SizedBox(height: 12),
                          const Divider(),
                          // Toplam Kazanç
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      decoration: BoxDecoration(
                        ...
                        border: Border.all(
                            color:
                                _getPerformanceColor().withValues(alpha: 0.3)),
                      ),
                      child: Column(
                        children: [
                          Row(
