...
class _BedenUretimTamamlaDialog extends StatefulWidget {
  ...
  @override
  State<_BedenUretimTamamlaDialog> createState() =>
      _BedenUretimTamamlaDialogState();
}
...
class _BedenUretimTamamlaDialogState extends State<_BedenUretimTamamlaDialog> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      final mevcutUretim =
          await _bedenService.getAsamaBedenTakip('dokuma', widget.atamaId);
  ...
  Future<void> _kaliteKontrolKaydiGuncelleVeyaOlustur({
    required int netAdet,
    required int toplamFire,
    required bool kismiKayit,
  }) async {
    final sourceKey = '[dokuma_atama_id:${widget.atamaId}]';
    final kismiNot = kismiKayit ? ' (Kısmi)' : '';
    final notlar =
        '$sourceKey Dokuma$kismiNot - ${widget.modelAdi} - $netAdet adet'
        '${toplamFire > 0 ? " (Fire: $toplamFire${firedenDus ? " düşüldü" : ""})" : ""}';

    final mevcut = await widget.supabase
        .from(DbTables.kaliteKontrolAtamalari)
        .select('id')
        .eq('firma_id', _firmaId)
        .eq('model_id', widget.atama['model_id'])
        .ilike('notlar', '%$sourceKey%')
        .maybeSingle();
    ...
    await widget.supabase.from(DbTables.kaliteKontrolAtamalari).insert(data);
  }
  ...
  Future<void> _kaydet({bool kismiKayit = false}) async {
    ...
    try {
      ...
      if (!kismiKayit && toplamUretilen + toplamFire < toplamHedef) {
        throw Exception(
          'Hedef adet tamamlanmadan üretim kapatılamaz. Eksik üretim için Kısmi Kaydet kullanın.',
        );
      }
          ...
          builder: (ctx) => AlertDialog(
            ...
            content: Column(
              ...
              children: [
                ...
                const SizedBox(height: 12),
                const Text(
                    'Fire miktarı sonraki aşamaya (Konfeksiyon) geçecek adetten düşülecektir.'),
      ...
      if (netAdet > 0) {
        try {
          final kismiNot = kismiKayit ? ' (Kısmi)' : '';
          await _kaliteKontrolKaydiGuncelleVeyaOlustur(
            netAdet: netAdet,
            toplamFire: toplamFire,
            kismiKayit: kismiKayit,
          );
          ...
          await BildirimService().roleGoreBildirimGonder(
            ...
            mesaj:
                '${widget.modelAdi} - $netAdet adet dokuma${kismiKayit ? " (kısmi)" : ""} tamamlandı.${toplamFire > 0 ? " (Fire: $toplamFire)" : ""}',
      ...
      if (!kismiKayit) {
        debugPrint(
            '🚀 Dokuma aşaması tamamlandı - Konfeksiyona adet aktarılıyor...');
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(mesaj),
            backgroundColor:
                yeniDurum == 'tamamlandi' ? Colors.green : Colors.orange,
          ),
        );
      }
    } catch (e) {
