...
extension _TabsRaporExt on _UretimRaporuPageState {
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        final kartlar = [
          ...
          _buildOzetKart('Devam Eden', _ozet['devam_eden'] ?? 0, Colors.orange,
              Icons.pending, isMobile),
          _buildOzetKart('Tamamlanan', _ozet['tamamlanan'] ?? 0, Colors.green,
              Icons.check_circle, isMobile),
          _buildOzetKart('Toplam Adet', _ozet['toplam_adet'] ?? 0,
              Colors.purple, Icons.numbers, isMobile),
          _buildOzetKart('Toplam Fire', _ozet['toplam_fire'] ?? 0,
              Colors.red.shade400, Icons.local_fire_department, isMobile),
          _buildOzetKart('Geciken', gecikenSiparis, Colors.red.shade700,
        ...
        child: ExpansionTile(
          ...
          subtitle: Column(
            ...
            children: [
              ...
              Text(
                  'Renk: ${model['renk'] ?? '-'} • Adet: ${model['adet'] ?? 0}'),
  ...
  Widget _buildAsamaProgress(
      Map<String, Map<String, dynamic>> asamalar, String mevcutAsama) {
      ...
      children: asamaListesiProgress.map((asama) {
        ...
        return Container(
          ...
          decoration: BoxDecoration(
            ...
            border:
                aktifAsama ? Border.all(color: Colors.black, width: 2) : null,
  ...
  Widget _buildAsamaDetayGrid(Map<String, Map<String, dynamic>> asamalar) {
    ...
    return GridView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        final talep =
            data['talep_edilen_adet'] ?? data['kontrol_edilecek_adet'] ?? 0;
        ...
        children: [
          ...
          Container(
            ...
            child: Text(
              durumMetin,
              style: TextStyle(
                  fontSize: 9, color: durumRenk, fontWeight: FontWeight.w600),
            ),
          ),
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              Text('$tamamlanan/$talep',
                  style: const TextStyle(
            ...
            children: [
              ...
              GridView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  final asamaInfo = asamaLabels[asamaKey] ??
                      {
                        'label': asamaKey,
                  ...
                  return Card(
                    ...
                    child: Container(
                      ...
                      child: Row(
                        children: [
                          Icon(asamaInfo['icon'] as IconData,
                          ...
                          Container(
                            padding: EdgeInsets.symmetric(
                                horizontal: isMobile ? 10 : 8, vertical: 4),
                            decoration: BoxDecoration(
                              color: oran > 5
                                  ? Colors.red.shade100
  ...
  Widget _buildEnCokFireModeller() {
    ...
    fireliModeller.sort((a, b) {
      int fireA = 0, fireB = 0;
      for (var asama
          in (a['asamalar'] as Map<String, Map<String, dynamic>>).values) {
        fireA += (asama['fire_adet'] ?? 0) as int;
      }
      for (var asama
          in (b['asamalar'] as Map<String, Map<String, dynamic>>).values) {
    ...
    return Card(
      child: ListView.separated(
        ...
        itemBuilder: (context, index) {
          ...
          int toplamFire = 0;
          for (var asama
              in (model['asamalar'] as Map<String, Map<String, dynamic>>)
                  .values) {
            toplamFire += (asama['fire_adet'] ?? 0) as int;
          }
  ...
  Widget _buildTerminTakibiTab() {
    ...
    final terminliModeller = _tumModeller.where((m) {
      ...
      return terminStr != null &&
          terminStr.isNotEmpty &&
          m['tamamlandi'] != true;
    }).toList();

    terminliModeller.sort((a, b) {
      final terminA =
          DateTime.tryParse(a['termin_tarihi'] ?? '') ?? DateTime(2100);
      final terminB =
          DateTime.tryParse(b['termin_tarihi'] ?? '') ?? DateTime(2100);
    ...
    final yaklasanlar = terminliModeller.where((m) {
      ...
      return termin != null &&
          termin.isAfter(now) &&
          termin.isBefore(now.add(const Duration(days: 7)));
    }).toList();
          ...
          child: Column(
            ...
            children: [
              ...
              isMobile
                  ? Column(
                      children: [
                        _buildTerminOzetKart('Geciken', gecikenler.length,
                            Colors.red, Icons.warning_amber),
                        const SizedBox(height: 8),
                        _buildTerminOzetKart('7 Gün İçinde', yaklasanlar.length,
                            Colors.orange, Icons.schedule),
                        const SizedBox(height: 8),
                        _buildTerminOzetKart('Normal', normal.length,
              ...
              if (terminliModeller.isEmpty)
                const Center(
                  child: Padding(
                    ...
                    child: Text(
                        'Termin tarihi tanımlı aktif sipariş bulunmamaktadır.'),
                  ),
                ),
            ],
          ),
    ...
    return Card(
      child: ListView.separated(
        ...
        itemBuilder: (context, index) {
          ...
          final tahmini =
              DateTime.tryParse(model['tahmini_tamamlanma']?.toString() ?? '');
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      child: Text(
                        ...
                        style: TextStyle(
                            fontSize: 10, color: mevcutAsama['color'] as Color),
                      ),
                    ),
                  ],
                ),
  ...
  Widget _buildTedarikciTab() {
    final tedarikciIstatistik =
        (_ozet['tedarikci_istatistik'] as Map<String, Map<String, dynamic>>?) ??
            {};
    ...
    final sirali = tedarikciIstatistik.entries.toList()
      ..sort((a, b) => ((b.value['toplam_model'] ?? 0) as int)
          .compareTo((a.value['toplam_model'] ?? 0) as int));
      ...
      child: Column(
        ...
        children: [
          ...
          ...sirali.map((entry) {
            ...
            final tedarikciModelleri = _tumModeller
                .where(
    ...
    return Card(
      child: ListView.separated(
        ...
        itemBuilder: (context, index) {
          ...
          final yuzde = (_tumModeller.isNotEmpty
                  ? entry.value / _tumModeller.length * 100
                  : 0)
              .toStringAsFixed(1);
          ...
          return ListTile(
            ...
            subtitle: LinearProgressIndicator(
              value: _tumModeller.isNotEmpty
                  ? entry.value / _tumModeller.length
                  : 0,
              backgroundColor: Colors.grey.shade200,
  ...
  Widget _buildKarsilastirmaTablosu(
      Map<String, dynamic> m1, Map<String, dynamic> m2) {
    final satirlar = <_KarsilastirmaSatir>[
      _KarsilastirmaSatir('Marka', m1['marka']?.toString() ?? '-',
          m2['marka']?.toString() ?? '-'),
      _KarsilastirmaSatir('Item No', m1['item_no']?.toString() ?? '-',
          m2['item_no']?.toString() ?? '-'),
      _KarsilastirmaSatir(
          'Renk', m1['renk']?.toString() ?? '-', m2['renk']?.toString() ?? '-'),
      _KarsilastirmaSatir(
          'Adet', m1['adet']?.toString() ?? '0', m2['adet']?.toString() ?? '0'),
      _KarsilastirmaSatir(
        'Mevcut Aşama',
        (_getAsamaBilgisi(m1['mevcut_asama'] ?? ''))['label'] as String,
        (_getAsamaBilgisi(m2['mevcut_asama'] ?? ''))['label'] as String,
      ),
      _KarsilastirmaSatir('Tedarikçi', m1['tedarikci_adi']?.toString() ?? '-',
          m2['tedarikci_adi']?.toString() ?? '-'),
    ];

    // Aşama bazlı fire karşılaştırma
    final asamalar1 =
        m1['asamalar'] as Map<String, Map<String, dynamic>>? ?? {};
    final asamalar2 =
        m2['asamalar'] as Map<String, Map<String, dynamic>>? ?? {};
    int fire1 = 0, fire2 = 0;
    for (var a in asamalar1.values) {
      fire1 += (a['fire_adet'] ?? 0) as int;
    }
    for (var a in asamalar2.values) {
      fire2 += (a['fire_adet'] ?? 0) as int;
    }
    satirlar.add(
        _KarsilastirmaSatir('Toplam Fire', fire1.toString(), fire2.toString()));
    ...
    satirlar.add(_KarsilastirmaSatir(
      'Termin Tarihi',
      termin1 != null
          ? DateFormat('dd/MM/yyyy').format(DateTime.parse(termin1))
          : '-',
      termin2 != null
          ? DateFormat('dd/MM/yyyy').format(DateTime.parse(termin2))
          : '-',
    ));
    ...
    satirlar.add(_KarsilastirmaSatir(
      'Tahmini Tamamlanma',
      tahmini1 != null
          ? DateFormat('dd/MM/yyyy').format(DateTime.parse(tahmini1))
          : '-',
      tahmini2 != null
          ? DateFormat('dd/MM/yyyy').format(DateTime.parse(tahmini2))
          : '-',
    ));
      ...
      children: [
        ...
        ...satirlar.map((s) => TableRow(
              children: [
                ...
                Padding(
                    padding: const EdgeInsets.all(8), child: Text(s.deger1)),
                Padding(
                    padding: const EdgeInsets.all(8), child: Text(s.deger2)),
              ],
            )),
      ],
    );
