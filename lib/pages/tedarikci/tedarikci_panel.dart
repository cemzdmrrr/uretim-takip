...
class _TedarikciPanelState extends State<TedarikciPanel>
  ...
  Future<void> _loadTedarikciData() async {
    try {
      ...
      debugPrint(
          '🔍 ${tedarikciInfo!['sirket']} tedarikci ID: ${tedarikciInfo!['id']} için atamalar aranıyor...');
      await _loadAtamalar();
    } catch (e) {
  ...
  Future<void> _loadAtamalar() async {
    ...
    try {
      ...
      for (String tabloAdi in atamaTablolari) {
        try {
          debugPrint(
              '🔍 $tabloAdi tablosunda tedarikci_id=$tedarikciId arıyor...');
          final response = await supabase.from(tabloAdi).select('''
          ...
          debugPrint(
              '📊 $tabloAdi tablosunda ${response.length} atama bulundu');
          for (var atama in response) {
            debugPrint(
                '   - Model: ${atama[DbTables.trikoTakip]['marka']} - ${atama[DbTables.trikoTakip]['item_no']}, Adet: ${atama['adet']}, Durum: ${atama['durum']}');
      ...
      setState(() {
        bekleyenAtamalar = tumAtamalar
            .where((a) => a['durum'] == 'atandi' || a['durum'] == 'beklemede')
            .toList();
        aktifAtamalar = tumAtamalar
            .where((a) =>
                a['durum'] == 'onaylandi' ||
                a['durum'] == 'uretimde' ||
                a['durum'] == 'baslatildi')
            .toList();
        tamamlananAtamalar = tumAtamalar
            .where((a) =>
                a['durum'] == 'tamamlandi' || a['durum'] == 'kismi_tamamlandi')
            .toList();
      });

      debugPrint(
          '✅ Atamalar yüklendi: ${bekleyenAtamalar.length} bekleyen, ${aktifAtamalar.length} aktif, ${tamamlananAtamalar.length} tamamlanan');

      // Test için atamaların durumlarını detaylı logla
      for (var atama in tumAtamalar) {
        debugPrint(
            '   📝 Atama ID: ${atama['id']}, Durum: ${atama['durum']}, Model: ${atama[DbTables.trikoTakip]['marka']}-${atama[DbTables.trikoTakip]['item_no']}');
      }
    } catch (e) {
  ...
  String _getAtamaTipiFromTable(String tabloAdi) {
    switch (tabloAdi) {
      case DbTables.dokumaAtamalari:
        return 'Dokuma';
      case DbTables.konfeksiyonAtamalari:
        return 'Konfeksiyon';
      case DbTables.nakisAtamalari:
        return 'Nakış';
      case DbTables.yikamaAtamalari:
        return 'Yıkama';
      case DbTables.ilikDugmeAtamalari:
        return 'İlik Düğme';
      case DbTables.utuAtamalari:
        return 'Ütü';
      default:
        return tabloAdi;
    }
          ...
          child: ListTile(
            ...
            subtitle: Column(
              ...
              children: [
                ...
                if (tip == 'bekleyen')
                  ...
                  Text(
                      'Kabul Edilen: ${atama['kabul_edilen_adet'] ?? atama['talep_edilen_adet'] ?? atama['adet'] ?? 0} adet'),
                  if (atama['tamamlanan_adet'] != null &&
                      atama['tamamlanan_adet'] > 0) ...[
                  ...
                  if (atama['tamamlama_tarihi'] != null)
                    Text(
                        '📅 Tamamlama: ${_formatDate(atama['tamamlama_tarihi'])}',
                        style:
            ...
            trailing: tip == 'bekleyen'
                ...
                : tip == 'aktif'
                    ...
                    : PopupMenuButton<String>(
                        onSelected: (value) =>
                            _handleTamamlananAction(value, atama),
                        itemBuilder: (context) => [
                          const PopupMenuItem(
                            value: 'revize',
                            child: ListTile(
