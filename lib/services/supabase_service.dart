...
class SupabaseService {
      ...
      for (var entry in data.entries) {
        ...
        if (existing != null) {
          ...
          await _client
              .from(DbTables.sistemAyarlari)
              .update(
                  {'deger': entry.value.toString(), 'guncelleme_tarihi': now})
              .eq('firma_id', _firmaId)
              .eq('anahtar', entry.key);
        } else {
          // Yeni kayıt ekle
          await _client.from(DbTables.sistemAyarlari).insert({
            'firma_id': _firmaId,
            'anahtar': entry.key,
  ...
  static Future<bool> updateTaxBrackets(
      List<Map<String, dynamic>> brackets) async {
    try {
      // Mevcut dilimleri sil
      await _client.from(DbTables.gelirVergisiDilimleri).delete().neq('id', 0);

      // Yeni dilimleri ekle
      final bracketsWithFirma =
          brackets.map((b) => {...b, 'firma_id': _firmaId}).toList();
      await _client
          .from(DbTables.gelirVergisiDilimleri)
          .insert(bracketsWithFirma);

      return true;
    } catch (e) {
      AppLogger.error(
          'SupabaseService', 'Vergi dilimleri güncelleme hatası', e);
      return false;
    }
  }
}
